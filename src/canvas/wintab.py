...
class WintabTabletCanvas(TabletCanvas):
    ...
    def _event_wt_packet(self, msg, wParam, lParam):
        ...
        altitude = packet.pkOrientation.orAltitude  # angle to XY-plane (0-3600 clockwise)
        azimuth = packet.pkOrientation.orAzimuth    # angle to Z-axis (900-0)
        buttons = packet.pkButtons                  # 

        title_bar_size = 30    # for windows7 default (Ugly Code!!)
        y += title_bar_size

        if 0 <= x <= self.window.width and 0 <= y <= self.window.height:
            packet2 = [x, y, pressure, azimuth, altitude, time.clock()]
            packet2 = [packet2] + [buttons]
            self.on_packet_func(packet2)

    def on_packet(self, func):
        self.on_packet_func = func 
...
def main():
    ...
    device = get_tablets()[0]
    tablet = WintabTabletCanvas(device, window)

    def show_packet(packet):
        print packet
    
    tablet.on_pressure(show_packet)
